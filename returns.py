...
# Read the csv file and display the data in a table sorted by quantity
df = pd.read_csv('returns.csv')

# Display the data in a table sorted by quantity and combine the sku and reason columns
st.table(df.groupby(['sku', 'reason'])['quantity'].sum().reset_index().sort_values(by='quantity', ascending=False).head(10))
